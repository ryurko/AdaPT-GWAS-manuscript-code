...
#          models returned. Then additionally generate the AdaPT intercept-only results
#          along with AdaPT results without any interactions allowed between 
#          the variables as a comparison to the optimal results presented.
...
results <- furrr::future_map_dfr(1:length(scz_variable_list),
                                 function(var_i) {
                                   ...
                                   adapt_results <- adapt_xgboost_cv(as.matrix(
                                     ...
                                     bip_scz_brainvar_data$scz_14_P,
                                   ...
                                   #         paste0("data/bip_schz_data/brainvar_results/cv_tune_results/scz_with_",
                                   ...
                                   do.call(rbind,
                                           lapply(target_alphas, 
                                                  function(alpha) {
                                                    ...
                                                    # Replicated nominal discoveries:
                                                    rep_disc <- which(bip_scz_brainvar_data$new_p_scz_18[adapt_disc] <= alpha)
                                                    ...
                                                    return(data.frame("method" = "adapt_cv",
                                                                      ...
                                                                      "n_rep_2018_disc" = length(rep_disc),
...
#                  "data/bip_schz_data/brainvar_results/cv_tune_results/scz_adapt_s05_2cv_summary.csv")
...
#         "data/bip_schz_data/brainvar_results/scz_intercept_only_s05.rds")


# Finally generate the results without any interactions allowed:
no_int_args <- list("nrounds" = 150,
                     "max_depth" = 3,
                     "min_child_weight" = 1,
                     "verbose" = 0,
                     "nthread" = 3,
                     # Restrict only 1 variable to be allowed in each tree
                     "colsample_bytree" = 1 / length(scz_variable_list$bd_z_eqtl_slopes_wgcna))


# Now create the results with no interactions
adapt_no_int_results <- adapt_xgboost(as.matrix(
  bip_scz_brainvar_data[,scz_variable_list$bd_z_eqtl_slopes_wgcna]),
  bip_scz_brainvar_data$scz_14_P,
  verbose = list(print = FALSE, 
                 fit = FALSE,
                 ms = FALSE),
...
# Save these results:
# saveRDS(adapt_no_int_results,
#         "data/bip_schz_data/brainvar_results/scz_all_vars_no_int_s05.rds")
