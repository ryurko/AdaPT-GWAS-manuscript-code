...
scz_brainvar_18_disc_snps <- bip_scz_brainvar_data$SNP[which(scz18_with_bd_z_eqtl_slopes_wgcna$qvals <= 0.05)]

# How many associated genes?
brainvar_eqtl_sheets$HCP_allEQTLs_FDR0.05 %>%
  filter(rsID %in% scz_brainvar_18_disc_snps) %>%
...
scz_brainvar_gene_file <- file("data/gene_lists/scz18_brainvar_disc_genes.txt")
brainvar_eqtl_sheets$HCP_allEQTLs_FDR0.05 %>%
  filter(rsID %in% scz_brainvar_18_disc_snps) %>%
  mutate(ensembl_id = str_sub(GeneId, 1, 15)) %>%
  pull(ensembl_id) %>%
