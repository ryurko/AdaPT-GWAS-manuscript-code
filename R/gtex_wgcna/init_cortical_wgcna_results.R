# PURPOSE: Generate WGCNA results for cortical tissues:
...
cortical_tpm_data <- readr::read_csv("data/gtex_eqtl/expression_datasets/tpm_cortical_samples.csv")
...
gtex_gene_table <- grex(cortical_tpm_data$clean_name)
...
cortical_tpm_data <- cortical_tpm_data[-zero_half_cortical_expression,]
# 17031
...
cortical_log_tpm <- apply(dplyr::select(cortical_tpm_data,
                                        -c(Name, clean_name,
                                           Description)),
...
colnames(cortical_log_tpm) <- cortical_tpm_data$Name
...
cortical_bwnet <- blockwiseModules(cortical_log_tpm, maxBlockSize = 20000,
...
cortical_wgcna_data <- data.frame("gene" = colnames(cortical_log_tpm),
                                  "wgcna_label" = cortical_mergedColors)

# Save them to the wcgna_results folder
# write_csv(cortical_wgcna_data, 
#          "data/gtex_eqtl/wcgna_results/cortical_wgcna_data.csv")
