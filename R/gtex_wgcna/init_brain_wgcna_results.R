# PURPOSE: Generate WGCNA results for additional brain tissues:
...
brain_tpm_data <- readr::read_csv("data/gtex_eqtl/expression_datasets/tpm_brain_samples.csv")
...
gtex_gene_table <- grex(brain_tpm_data$clean_name)
...
zero_half_brain_expression <- which(apply(dplyr::select(brain_tpm_data,
                                                           -c(Name, clean_name,
                                                              Description)), 1,
                                             function(x) length(which(x == 0)) / length(x) >= .5))

# Now remove these genes
brain_tpm_data <- brain_tpm_data[-zero_half_brain_expression,]
# 17031
...
brain_log_tpm <- apply(dplyr::select(zero_half_brain_expression,
                                        -c(Name, clean_name,
                                           Description)),
...
colnames(brain_log_tpm) <- brain_tpm_data$Name
...
brain_power_search <- pickSoftThreshold(brain_log_tpm, 
                                           dataIsExpr = TRUE, 
                                           powerVector = powers,
                                           corFnc = cor,
...
brain_bwnet <- blockwiseModules(brain_log_tpm, maxBlockSize = 20000,
                                   power = 7, TOMType = "unsigned", minModuleSize = 30,
                                   numericLabels = TRUE,
                                   saveTOMs = TRUE,
                                   saveTOMFileBase = "brain_TOM_blockwise",
                                   verbose = 3)
#setwd("data")

# Convert labels to colors for plotting
brain_mergedColors <- labels2colors(brain_bwnet$colors)

# Create a dataframes with the gene names and the labels:
brain_wgcna_data <- data.frame("gene" = colnames(brain_log_tpm),
                                  "wgcna_label" = brain_mergedColors)

# Save them to the wcgna_results folder
# write_csv(brain_wgcna_data, 
#          "data/gtex_eqtl/wcgna_results/brain_wgcna_data.csv")
